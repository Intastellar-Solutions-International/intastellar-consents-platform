    ...
    return (
        <>
            <div className={"widget" + overViewTotal}>
                ...
                <p>{props?.type}</p>
                {
                    (props?.totalNumber?.dailyNum) ? <Line title="Consents giving" data={props?.totalNumber?.dailyNum} data2={props?.totalNumber?.dailyNum} fromDate={props?.fromDate} toDate={props?.toDate} /> : null
                }
            </div>
        </>
    )
